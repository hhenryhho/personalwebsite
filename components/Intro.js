...
const Intro = () => {
  ...
  return (
    <Flex flexDir="column" pt={[50, 150]} pb={[100, 300]} id="intro">
      <Flex m="auto" w="90%" flexDir="column" alignItems="center">
        <Text fontSize={['md', 'lg']}>Hi, my name is </Text>
        <Heading fontSize={['3xl', '4.3rem']}>Henry Ho.</Heading>
        <Text textAlign="center" fontSize={['md', 'lg']}>
          I am a software developer with a passion for full-stack development.
        </Text>
        <Text textAlign="center" fontSize={['md', 'lg']}>
          I love to learn new things and I am constantly exploring new
          technology.
        </Text>
      </Flex>
      <Flex m="auto" position="relative">
        <Stack
          ...
          {contactButtonGroupItems.map((item, index) => (
            <Box key={index} align={isLargeScreen ? 'start' : 'center'}>
              {item}
            </Box>
          ))}
        </Stack>
